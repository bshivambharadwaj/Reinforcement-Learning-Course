# GoalLab foundations: the two-source teaching environment
...
A [live visual demo](foundations-demo.html) and [reference benchmark records](results.json) are included. GitHub shows HTML source, so download the HTML and open it locally. Click **Run agent**, pause, or advance one step at a time. The browser executes the local workspace: source cards reveal reads, candidate branches appear during search, the briefing changes as decisions are made, and final checks show whether the submitted result is supported.
...
- [Notebook 08](apply-theory/08_one_problem_many_algorithms.ipynb): shared-problem algorithm comparisons.
- [Notebook 09](apply-theory/09_small_model_sft_dpo.ipynb) and [Notebook 10](apply-theory/10_verifiable_reasoning_grpo.ipynb): preference learning and verifiable training feedback.
- [Notebook 11](apply-theory/11_tool_gateway_integration.ipynb): GoalLab gateway integration and evaluator gaming.
- [Notebook 12](apply-theory/12_sampling_and_selection.ipynb), [Notebook 13](apply-theory/13_budgeted_reasoning_search.ipynb), and [Notebook 14](apply-theory/14_learning_from_search.ipynb): selection, search, and a separate learning stage.
