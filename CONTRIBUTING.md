...
Course notebooks should use the clearest environment for the algorithm, preserving their diagrams, worked examples, and comparisons. Project notebooks under `projects/goallab/apply-theory/` solve stages of [GoalLab](projects/goallab/STAGES.md), using its shared workspace and briefing evaluator. Maintain reciprocal links between the two tracks. Keep its input, output, algorithm role, and worked constraint-changing exercise explicit. Broader environment families should be deliberate project extensions with their own contracts.
